...
1. Confirm the committed workflow is present on the target branch and passed disposable
   validation.
2. Keep existing tasks on their current workflow; do not trigger a task migration.
...
4. Run `./agent-vm configure-kandev-workflow`. Require a successful result with
   `last_ok`, no error, and no warnings. The configurator renames the manual duplicate
   to `Development (legacy)` and places synced `Development` first for new tasks.
